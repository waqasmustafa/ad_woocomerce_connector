...
class WooProduct(models.Model):
    ...
    def _sync_one(self, backend, record: dict, force: bool = False, parent_tmpl=None, existing_product=None):
        ...
        if existing:
            ...
            if images:
                ...
                if existing.is_variation:
                    if not existing.product_id.image_variant_1920:
                        img_b64 = _fetch_image_b64(first_url)
                        if img_b64:
                            existing.product_id.with_context(syncing_from_wc=True).write(
                                {"image_variant_1920": img_b64}
                            )
                elif not existing.product_id.product_tmpl_id.image_1920:
                    img_b64 = _fetch_image_b64(first_url)
                    if img_b64:
        ...
        if sku and backend.match_product_by_sku:
            ...
            if parent_tmpl:
                domain.append(("product_tmpl_id", "=", parent_tmpl.id))
        ...
        if not odoo_product:
            ...
            if existing_product:
                # Use the Odoo variant already matched by attribute values
                ...
                unused = parent_tmpl.product_variant_ids.filtered(
                    lambda v: not v.default_code and not self.env["wc.product.link"].search(
                        [("product_id", "=", v.id)], limit=1
                    )
                )
                if unused:
...
class WooProductTemplate(models.Model):
    ...
    def _setup_variant_attributes(self, odoo_tmpl, var_records, parent_name):
        """Create attribute lines on the template from WooCommerce variation attributes.
        Returns attr_value_map: {attr_name: {option: attribute_value_id}}
        """
        # Collect all attributes across all variations
        all_attrs = {}
        for var in var_records:
            for attr in var.get("attributes", []):
                aname = (attr.get("name") or "").strip()
                option = (attr.get("option") or "").strip()
                if aname and option and option not in all_attrs.get(aname, []):
                    all_attrs.setdefault(aname, []).append(option)

        # Fallback: no attributes defined → use generic "Option" attribute
        if not all_attrs:
            options = []
            for var in var_records:
                vname = (var.get("name") or "").strip()
                opt = vname.replace(parent_name, "").strip(" -") or str(var.get("id", ""))
                if opt and opt not in options:
                    options.append(opt)
            if options:
                all_attrs["Option"] = options
        ...
        for aname, options in all_attrs.items():
            ...
            for option in options:
                ...
                if not val:
                    val = self.env["product.attribute.value"].with_context(
                        syncing_from_wc=True
                    ).create({"attribute_id": attr.id, "name": option})
                val_ids.append(val.id)
                attr_value_map[aname][option] = val.id
            ...
            if attr_line:
                new_ids = [v for v in val_ids if v not in attr_line.value_ids.ids]
                if new_ids:
                    attr_line.write({"value_ids": [(4, v) for v in new_ids]})
            else:
    ...
    def _match_variant_by_attrs(self, odoo_tmpl, var_attrs, attr_value_map):
        """Find the product.product whose attribute combination matches the variation."""
        target_ptav_ids = set()
        for vattr in var_attrs:
            ...
            if aname in attr_value_map and option in attr_value_map[aname]:
                val_id = attr_value_map[aname][option]
                ptav = self.env["product.template.attribute.value"].search([
                    ("product_tmpl_id", "=", odoo_tmpl.id),
                    ("product_attribute_value_id", "=", val_id),
                ], limit=1)
                if ptav:
                    target_ptav_ids.add(ptav.id)
    ...
    def _sync_variations(self, backend, tmpl_binding, record: dict, force: bool, results=None):
        ...
        # Enrich variation records with name/categories/tags
        for var in var_records:
            if not var.get("name"):
                attr_parts = [
                    a.get("option", "") for a in var.get("attributes", []) if a.get("option")
                ]
                var["name"] = parent_name + (" - " + ", ".join(attr_parts) if attr_parts else "")
            if not var.get("categories"):
                var["categories"] = record.get("categories", [])
            if not var.get("tags"):
                var["tags"] = record.get("tags", [])

        # Set up attribute lines on template and get value map
        try:
            attr_value_map = self._setup_variant_attributes(odoo_tmpl, var_records, parent_name)
        except Exception as exc:
            _logger.warning("Could not set up attributes for template %s: %s", odoo_tmpl.id, exc)
            attr_value_map = {}
        ...
        for var in var_records:
            ...
            # Find matching Odoo variant by attribute values
            existing_product = None
            var_attrs = var.get("attributes", [])
            if var_attrs and attr_value_map:
                existing_product = self._match_variant_by_attrs(odoo_tmpl, var_attrs, attr_value_map)

            try:
                var_binding, status = wc_product_model._sync_one(
                    backend, var, force,
                    parent_tmpl=odoo_tmpl,
                    existing_product=existing_product,
                )
                if results is not None and status:
                    results[status] += 1
                if var_binding:
        ...
        # Set per-variant prices using price_extra on PTAVs
        try:
            self._update_variant_prices(odoo_tmpl, var_records, attr_value_map)
        except Exception as exc:
            _logger.warning("Could not update variant prices for template %s: %s", odoo_tmpl.id, exc)

    def _update_variant_prices(self, odoo_tmpl, var_records, attr_value_map):
        """Set price_extra on product.template.attribute.value so each variant
        shows its correct WooCommerce price in the Odoo Sales Price column."""
